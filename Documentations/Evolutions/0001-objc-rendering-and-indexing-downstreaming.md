...
| 2026-08-10 | **`OutputTransformer` 只把抽象骨架留在 `swift-semantic-string`** | 本提案原方案是整包并入。落地后收紧：通用字符串包只留 `Transformer` 命名空间与 `Module` 协议（51 行），五个 Swift 模块进 MachOSwiftSection 的 `SwiftOutputTransformer`，两个 ObjC 模块进 MachOObjCSection 的 `ObjCOutputTransformer`，聚合 `Configuration` 回 RuntimeViewerCore（唯一使用者）。理由就是本提案否决「并进 `Semantic` target」时用的那一条 —— token 名字带着领域词汇表；这次推到底。命名空间必须留在通用包，否则两边同名 `Transformer` 在 RuntimeViewer 里歧义 |
